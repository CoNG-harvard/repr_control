...
### This is a the same as kuramoto v3, just that rewards are local wrt to fixed target. ###
...
env_name = 'kuramoto_w_kappa_and_target'
...
adjacency = build_adjacency(N,kappa = 1)
...
def dynamics(state: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
    ...
    # print("val",torch.sum(sin_th_diff * P, axis = -1))
...
def rewards(state: torch.Tensor, action: torch.Tensor, target = 1.0) -> torch.Tensor:
    """
    ...
    th_diff  = th - target
    # th_diff_proj = (th_diff + torch.pi) % (2 * torch.pi)  - torch.pi

    # print("th_diff_proj", th_diff_proj)
