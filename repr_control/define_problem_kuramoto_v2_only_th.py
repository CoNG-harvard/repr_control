...
state_range = [[-np.pi] * N,
               [np.pi] * N]           # low and high. We set bound on the state to ensure stable training.
action_range = [[-4] * N, [4] * N]          # low and high
max_step = 500                      # maximum rollout steps per episode
sigma = 0.05                      # noise standard deviation.
env_name = 'kuramoto_w_kappa'
...
adjacency = build_adjacency(N,kappa)
...
kappa_obs_dim = (2*kappa + 1) * state_dim #this is the dimension of the concatenation of the states of an agent's kappa-neighborhood neighbors
eval_kappa_obs_dim = (2*kappa + 1) * state_dim
eval_kappa_action_dim = (2*kappa + 1) * action_dim
...
def dynamics(state: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
    ...
    newth = state + dt * thdot

    next_state = (newth + torch.pi) % (2 * torch.pi) - torch.pi #project onto [-pi,pi]
