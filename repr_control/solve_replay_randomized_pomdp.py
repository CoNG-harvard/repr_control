...
from repr_control.utils import util, buffer, buffer_vec_pomdp
...
from define_problem_burgers import *
# from define_problem_pomdp import *
...
if __name__ == "__main__":
    ...
    ### parameter that
    parser.add_argument("--alg", default="svd_sac_pomdp",
    ...
    parser.add_argument("--rsvd_num", default=1024, type=int,
    ...
    parser.add_argument("--start_timesteps", default=100, type=float,
    ...
    parser.add_argument("--has_feature_step", action='store_true',
                        help="Enable the feature step in training.")
    parser.add_argument("--no_feature_step", dest='has_feature_step', action='store_false',
                        help="Disable the feature step in training.")
    parser.set_defaults(has_feature_step=False)
    ...
    use_V_critic = False
    has_feature_step = args.has_feature_step
    log_path = f'log/replay/{alg_name}/{env_name}/start_timesteps={args.start_timesteps}/sigma={sigma}/rsvd_sigma={args.rsvd_sigma}/V_critic={use_V_critic}/feature_step={has_feature_step}/{exp_name}'
    ...
    replay_buffer = buffer_vec_pomdp.ReplayBuffer(state_dim, obs_dim,action_dim, max_size = int(args.max_timesteps * args.batch_size), device=args.device)
    ...
    best_actor = None
    best_critic_phi = None
    best_critic_last_layer = None
    ...
    for t in range(int(args.max_timesteps + args.start_timesteps)):
        ...
        elif episode_timesteps <= 0: #use zero action until control is really needed.
            action = torch.zeros((args.batch_size,1), device=torch.device(args.device))
        else:
            action = agent.batch_select_action(obs, explore=True)
            # print("action here", action[-3:,:])


        # Perform action
        next_state, reward, terminated, truncated, rollout_info = env.step(action)
        if episode_timesteps % 50 == 0:
            # print("reward in solve, t = %d" %episode_timesteps, reward)
            pass

        obs = state[:,ctrl_pos - int(obs_dim/2):ctrl_pos+int(obs_dim/2)]
        next_obs = next_state[:,ctrl_pos- int(obs_dim/2):ctrl_pos+int(obs_dim/2)]
        # print("next obs shape in solve", next_obs.shape)
        done = truncated
        if episode_timesteps > 40: #only add data once obs is starting to become different from target
            replay_buffer.add(state, obs,action, next_state, next_obs,reward, done)
        ...
        if t > args.start_timesteps: #only train after some initial time.
        ...
        if (t + 1) % args.eval_freq == 0:
            ...
            eval_len, eval_ret, _, _ = util.batch_eval_burgers(agent, eval_env, ctrl_pos = ctrl_pos, obs_dim = obs_dim)
            ...
            if eval_ret > best_eval_reward:
                ...
                best_critic_last_layer = agent.critic_last_layer.state_dict()
                best_critic_phi = agent.phi_net.state_dict()

                # save best actor/best critic
                torch.save(best_actor, log_path + "/best_actor.pth")
                torch.save(best_critic_last_layer, log_path + "/best_critic_last_layer.pth")
                torch.save(best_critic_phi, log_path + "/best_critic_phi.pth")
    ...
    torch.save(agent.critic_last_layer.state_dict(), log_path + "/critic_last_layer_last.pth")
    torch.save(agent.phi_net.state_dict(), log_path + "/critic_phi_last.pth")
