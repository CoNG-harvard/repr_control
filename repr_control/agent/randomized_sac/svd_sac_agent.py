...
class svdSACAgent(object):
	...
	def __init__(
		...
		self.nn_mu_net = learn_phi.nnMu(state_dim, output_dim = self.rsvd_num, hidden_dim = 256).to(self.device)
	...
	def feature_step(self, batch, batch2):
		...
		state2, action2, next_state2, reward2, done2 = unpack_batch(batch2)
	

		phi = self.phi_net(state,action)
		mu_self = self.nn_mu_net(next_state)
		# mu_contrast = self.nn_mu_net(shuffled_state)
		mu_contrast = self.nn_mu_net(state2)
