...
class CustomPhiSACAgent(SACAgent):
	...
	def __init__(
		...
		output_dim=rsvd_num, hidden_dim=hidden_dim).to(self.device) for _ in range(self.N)]
		self.rand_mu_nets = [RandMu(eval_kappa_obs_dim, output_dim=rsvd_num, sigma=1.0).to(self.device) for _ in range(self.N)]
	...
	def fmodel_phi(self,params,buffers,x):
		return functional_call(self.base_phi,(params,buffers),(x,))
		...
		for i in range(self.N):
			...
			for name, param in self.critic_last_layers[i].named_parameters():
				if param.requires_grad == True:
					...
					param.data = 0.9 * torch.ones(size = param.data.size()).to(self.device)
