...
class CustomModelRFSACAgent(SACAgent):
    ...
    def __init__(self,
        ...
        self.actor_optimizers = [torch.optim.Adam(
            self.actors[i].parameters(), lr=lr, betas=[0.9, 0.999]
        ) for i in range(self.N)]


        self.log_alphas = [torch.tensor(np.log(alpha), device = self.device, requires_grad = True) for i in range(self.N)]
        

        self.log_alpha_optimizers = [torch.optim.Adam(
            [self.log_alphas[i]], lr=lr, betas=[0.9, 0.999]
        ) for i in range(self.N)]


    @property #this overwrites the other function in the sac file
    def alphas(self):
        alphas = torch.zeros(self.N).to(self.device)
        for i in range(self.N):
            alphas[i] = self.log_alphas[i].exp()
    ...
    def update_actor_and_alpha(self, batch):
        ...
        for key in actor_params.keys():
            if actor_params[key].requires_grad == True:
                ...
                # print("key", key)
                # print("length of value of the key", len(actor_params[key]))

        
        new_actor_optimizers = torch.optim.Adam(
        ...
        actor_loss = 0
        for i in range(self.N):
            # actor_losses_i = (self.alphas/self.N * log_prob_vmap - q)[:,i]
            actor_losses_i = (self.alphas * log_prob_vmap - q)[:,i]
            actor_loss_i = torch.sum(actor_losses_i)
            actor_loss += actor_loss_i
            self.actor_optimizers[i].zero_grad()
            new_actor_optimizers.zero_grad() 
            actor_loss_i.backward(retain_graph = True)
            #basically only update grad for agent i when computing loss_i, even though grad i is also calculated for other losses
        ...
        if self.learnable_temperature:
            ...
            for i in range(self.N):
                ...
                alpha_losses_i = (self.log_alphas[i] *
                            (-log_prob_vmap - self.target_entropy).detach())[:,i]
                alpha_loss_i = torch.sum(alpha_losses_i)
    ...
    def critic_step(self, batch):
        ...
        # critic_norm_norm = 0
        # for i in range(self.N):
        #     critic_norm_norm += self.critics[i].norm1.weight.norm()
        # # print("this is critic layer norm", critic_norm_norm)
        ...
        info = {
            ...
            # 'layer_norm_weights_norm': critic_norm_norm,
        }
