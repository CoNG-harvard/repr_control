...
# from repr_control.agent.rfsac import rfsac_agent_network
...
if __name__ == "__main__":
    ...
    ### parameter that
    parser.add_argument("--alg", default="sac_v2",
    ...
    kwargs.update({
        ...
        'device': curr_device, #device from the define_problem_*.py.
        'critic_hidden_dim': critic_hidden_dim, #try smaller critic hidden dim
    })
    ...
    log_path = f'log/replay/{alg_name}/{env_name}/N={N}/sigma={sigma}/sac_critic_hidden_dim={critic_hidden_dim}/batchsize={args.batch_size}/critic_use_ortho_init={args.critic_use_ortho_init}/tau={args.tau}/{exp_name}'
    ...
    # Initialize policy
    if args.alg == "sac_v2":
        agent = sac_agent_network_vmap_v2.CustomModelSACAgent(dynamics_fn = dynamics, rewards_fn = rewards, **kwargs)
    elif args.alg == 'rfsac':
        agent = rfsac_agent_network.CustomModelRFSACAgent(dynamics_fn = dynamics, rewards_fn = rewards, **kwargs)
        # agent = normal_sac_agent_network_vmap.CustomModelSACAgent(dynamics_fn = dynamics, rewards_fn = rewards, **kwargs)
    else:
        raise NotImplementedError("Algorithm not implemented.")

    replay_buffer = buffer_vec_network.ReplayBuffer(state_dim, action_dim, max_size = int(args.max_timesteps * args.batch_size), N = N,device=args.device)
